...
- Gemma 1B polish goes off the rails on junk input (chats back, duplicates with garbage joints like "ieux", switches language, fabricates URLs like paste.org). ScriptPreparer guards: junk input skip, length ratio 1.4x, word overlap 0.5, repeated 4 gram detector, novel word fabrication check with a spoken form whitelist, foreign script check, adaptive token cap. Keep all of them; each caught a real live failure.
- Debug Mode (Preferences toggle, off by default) writes DEBUG entries into the log: every chunk sent to TTS plus polish in and out, viewable live in the Logs panel of the app window. DebugTrace.append is a no op when off; spoken content reaches disk only while on, so it stays off in normal use for privacy.
- Logs rotate daily: one file per day at ~/Library/Logs/ReadMe/ReadMe-<day>.log, files from other days are deleted on write. The app window (MainWindow.swift) is a sidebar plus detail panel holding Settings, Logs, and About; new panels go there, not in new windows. The status menu only links to Preferences; Logs and About are reached through the sidebar.
